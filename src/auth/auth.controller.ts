...
export class AuthController {
    ...
    async googleCallback(@Req() req, @Res() res: Response) {
        console.log('🔙 [GoogleCallback] Received callback');

        const user = req.user;
        if (!user) {
            console.error('❌ [GoogleCallback] No user in request');
            throw new UnauthorizedException({
                code: ERROR_CODE.GOOGLE_UNAUTHORIZED,
            });
        }
        console.log('✅ [GoogleCallback] User:', user.email, 'ID:', user.id);

        const { state } = req.query;
        if (!state) {
            console.error('❌ [GoogleCallback] No state in query');
            throw new UnauthorizedException({
                code: ERROR_CODE.GOOGLE_UNAUTHORIZED,
            });
        }
        ...
        let deviceInfo;
        try {
            deviceInfo = JSON.parse(Buffer.from(state as string, 'base64').toString());
            console.log('📱 [GoogleCallback] Device Info:', deviceInfo);
            console.log('📱 [GoogleCallback] Raw state:', state);
        } catch (error) {
            console.error('❌ [GoogleCallback] Failed to parse state:', error);
            // ✅ Fallback: Tạo device info mặc định
            deviceInfo = {};
        }

        // ✅ Validate device_id, nếu không có thì tạo mặc định
        if (!deviceInfo.device_id) {
            console.warn('⚠️ [GoogleCallback] Missing device_id in state, creating default');
            console.warn('⚠️ [GoogleCallback] State was:', state);
            console.warn('⚠️ [GoogleCallback] Parsed deviceInfo:', deviceInfo);

            // Tạo device_id mặc định cho Google OAuth
            // Format: google_oauth_{user_id}_{timestamp}
            const timestamp = Date.now();
            deviceInfo = {
                device_id: `google_oauth_${user.id}_${timestamp}`,
                device_name: 'Google OAuth Device',
                device_type: 'mobile',
            };

            console.log('✅ [GoogleCallback] Created default device info:', deviceInfo);
        }
        ...
        if (isWeb) {
            ...
            console.log('🌐 [GoogleCallback] Redirecting to web:', redirectUrl);
            return res.redirect(redirectUrl);
        }

        const redirectUrl = `${process.env.MOBILE_URL}auth/callback?code=${code}`;
        console.log('📱 [GoogleCallback] Redirecting to mobile:', redirectUrl);
        console.log('📱 [GoogleCallback] MOBILE_URL from env:', process.env.MOBILE_URL);
        return res.redirect(redirectUrl);
    }
