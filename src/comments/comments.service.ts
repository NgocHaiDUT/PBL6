...
export class CommentsService {
  ...
  async create(createCommentDto: CreateCommentDto, userId: number): Promise<CommentResponse> {
    ...
    if (createCommentDto.parent_id) {
      ...
      if (parentComment.target_type !== createCommentDto.target_type || 
          parentComment.target_id !== createCommentDto.target_id) {
        throw new ForbiddenException('Parent comment does not belong to the same target');
      }
    }
  ...
  async findAll(queryDto: QueryCommentsDto): Promise<PaginatedCommentsResponse> {
    const { target_type, target_id, parent_id, page = 1, limit = 20, include_replies, user_id } = queryDto;
    ...
    const addLikeStats = async (comment: any): Promise<any> => {
      ...
      const [likes_count, is_liked] = await Promise.all([
        ...
        user_id ? this.prisma.likes.findUnique({
          where: {
  ...
  async update(id: number, updateCommentDto: UpdateCommentDto, userId: number): Promise<CommentResponse> {
